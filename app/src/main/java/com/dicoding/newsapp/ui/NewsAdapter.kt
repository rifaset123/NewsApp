...
class NewsAdapter(private val onBookmarkClick: (NewsEntity) -> Unit) : ListAdapter<NewsEntity, MyViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        // memeriksa status berita, apakah sudah ditandai atau tidak

        /*
                        IMPORTANT NOTE
        Kenapa di onBindViewHolder ? bukan di ViewHolder
        Karena ingin menyimpan state dari bookmark yang ditandai.

        Apabila Anda meletakkan kode ini di ViewHolder,
        maka state akan ikut ter-recycle (daur ulang) yang menyebabkan tanda tidak semestinya.
        ...
        ivBookmark.setOnClickListener {
            // lambda yang digunakan seperti callback untuk melakukan aksi di luar adapter
